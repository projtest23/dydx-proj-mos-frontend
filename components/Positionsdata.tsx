...
// Cleanup the interval on component unmount
  ...
  return (
    ...
    <div className={pos.long?'poslong':'posshort'}>
      ...
      <div className='size'>
        <p>{pos.size}</p>
        <span className='leverage'>{pos.leverage}×</span>
        </div>

      <div className={pos.unrealized_profit<0?'Unrealizedshort':'Unrealizedlong'}>
        <p>{pos.unrealized_profit<0?'-$'+(-pos.unrealized_profit):'$'+pos.unrealized_profit}</p>
        <span className={pos.realized_PL<0?'Realizedshort':'Realizedlong'}>{pos.realized_PL<0?'-$'+(-pos.realized_PL):
        '$'+pos.realized_PL}</span>
        </div>

      <div className='Liq'>
        <p>${pos.liq_price}</p>
        <span className='Oracle'>${pos.oracle}</span>
        </div>
    </div>
    </div>
  </div>
